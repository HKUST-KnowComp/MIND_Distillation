...
def prepare_csv(df, csv_path):
    ...
    for id, row in exist_name.iterrows():
        ...
        df['Agreement'].append(row['Agreement'])
    return df
...
def main(args):
    ...
    image = args.image
    resume = args.resume
    csv_path = 'path2intention'
    labelled_image_path = 'path2img'
    save_path = 'save_path'
    ...
    new_df = {'id_a': [], 'id_b': [], 'Product_a': [], 'Product_b': [], 'Relation': [],
              'product_a_desc': [], 'product_b_desc': [], 'Intention': [], 'tail': [],
              'Agreement': []} 
    start_id = -100
    if start_id > 0:
    ...
    for id, row in tqdm(df.iterrows(), desc=f'Name Clean Process {portion}', total=len(df)):
        ...
        if "mpt" in model_name.lower():
            ...
            roles = conv.roles
        ans_lst = []
        product_a_id = row['id_a']
        product_b_id = row['id_b']
        product_a_intent = row['product_a_desc']
        product_b_intent = row['product_b_desc']
        prod_a_desc = row['Product_a']
        prod_b_desc = row['Product_b']
        ...
        question_lst = [
            f'The two images are two different products. The product name of the upper image is {prod_a_desc}. The product detail and the potential purchase intention is {product_a_intent}. The product name of the lower image is {prod_b_desc}. The product detail and the potential purchase intention is {product_b_intent}. Under the relation of {relation_prompt[relation]}, the potential co-buy intention would be {intention}. If you are a consumer who are eager to buy product a or product b, would this intention encourage you to buy the two products simultaneously? be critical on your choice, output yes or no together with the reason for your answer. For example, the output should be Yes, ... or No, ...'
            ]
        image_a, image_b = load_image(os.path.join(product_a_dir, os.listdir(product_a_dir)[0])), load_image(
            os.path.join(product_b_dir, os.listdir(product_b_dir)[0]))
